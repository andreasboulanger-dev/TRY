...
function initFarmMap() {
  window.farmMap = farmMap = L.map('farmMap', {center:[-30,147], zoom:4, maxZoom:13, zoomControl:false});
...
// ── Shared sanitisers (also used by openFarmDetail below) ──
// Prevent javascript: injection / malformed values in href attributes.
...
// Small round contact-icon buttons for Telephone / Email / Website /
// Instagram / Facebook / WhatsApp, shown below the category pills. Each
// button only appears if the underlying data exists — the farm data model
// currently has phone/email/website/fb_link; Instagram has no dedicated
// field yet so it renders automatically once one exists (f.instagram or
// f.instagram_link). WhatsApp has no separate field either — it's derived
// from the phone number (wa.me link) since that's the number most farms
// actually use for WhatsApp.
function buildFarmContactIcons(f) {
  const phone     = safePhone(f.phone);
  const email     = safeEmail(f.email);
  const website   = safeUrl(f.website);
  const facebook  = safeUrl(f.fb_link);
  const instagram = safeUrl(f.instagram_link || f.instagram);
  const waDigits  = phone ? phone.replace(/[^\d]/g, '') : '';

  const items = [];
  if (phone)     items.push(`<a class="contact-icon-btn tel" href="tel:${escHTML(phone)}" aria-label="Call"><i class="ti ti-phone"></i></a>`);
  if (email)     items.push(`<a class="contact-icon-btn email" href="mailto:${escHTML(email)}" aria-label="Email"><i class="ti ti-mail"></i></a>`);
  if (website)   items.push(`<a class="contact-icon-btn website" href="${escHTML(website)}" target="_blank" rel="noopener noreferrer" aria-label="Website"><i class="ti ti-world"></i></a>`);
  if (instagram) items.push(`<a class="contact-icon-btn instagram" href="${escHTML(instagram)}" target="_blank" rel="noopener noreferrer" aria-label="Instagram"><i class="ti ti-brand-instagram"></i></a>`);
  if (facebook)  items.push(`<a class="contact-icon-btn facebook" href="${escHTML(facebook)}" target="_blank" rel="noopener noreferrer" aria-label="Facebook"><i class="ti ti-brand-facebook"></i></a>`);
  if (waDigits)  items.push(`<a class="contact-icon-btn whatsapp" href="https://wa.me/${escHTML(waDigits)}" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp"><i class="ti ti-brand-whatsapp"></i></a>`);

  return items.length ? `<div class="popup-contact-icons">${items.join('')}</div>` : '';
}

// Shared builder — same popup markup used for a single farm pin AND for
// each slide of the cluster carousel below, so a farm's card looks
// identical no matter how you got to it.
function buildFarmPopupHTML(f) {
  ...
  const contactIcons = buildFarmContactIcons(f);
  return `<div class="popup-inner"><div class="popup-name">${escHTML(f.business_name)}${verifiedIcon}</div><div class="popup-detail" style="display:flex;align-items:center;gap:6px;">${escHTML(f.city||'')}${f.state_region?', '+escHTML(f.state_region):''} · ${escHTML(f.country||'')}<span id="popup-stars-${f.id}" style="display:inline-flex;align-items:center;gap:2px;margin-left:2px;"></span></div><div class="popup-badges"><span class="badge">${escHTML(f.farm_category_first||'Farm')}</span>${pay}${acc}${is88popup}</div>${contactIcons}${f.notes?`<div style="font-size:12px;color:var(--text-light);margin-bottom:10px;">${escHTML(f.notes)}</div>`:''}<button class="popup-btn" onclick="openFarmDetailFromPopup('${f.id}')">View details</button></div>`;
}

// Loads/fills the star-rating span inside a popup built by buildFarmPopupHTML above.
function loadFarmPopupStars(f) {
...
function addFarmMarkers(farms) {
  if (!farmCluster) {
    ...
    // Clicking a cluster normally zooms in toward its bounds
    // (zoomToBoundsOnClick above). But `clusterclick` still fires even when
    // the map is already at max zoom and can't zoom in any further — in
    // that case (with spiderfyOnMaxZoom off), show a swappable carousel of
    // standard farm cards for every marker inside the cluster instead.
    farmCluster.on('clusterclick', function(a) {
      if (farmMap.getZoom() >= farmMap.getMaxZoom()) {
        openFarmClusterCard(a.layer.getAllChildMarkers());
      }
    });
  }
  farms.forEach(f => {
    ...
    const popupHTML = buildFarmPopupHTML(f);
    const marker = L.marker([lat,lng],{icon});
    marker.farmData = f;
    marker.on('click', function() {
      openPinCard(popupHTML, { hideClose: true, onOpen: () => loadFarmPopupStars(f) });
    });
...
// Opens the pin card with a swappable carousel of standard farm popup
// cards — the exact same card you'd see tapping a single farm pin — one
// slide per marker inside the clicked cluster. Swipe to move between
// farms; small dots at the bottom show where you are (only shown when
// there's more than one). No close button of its own — tap outside to
// dismiss, same as any other pin card.
let _clusterCarouselEl = null;

function openFarmClusterCard(markers) {
  const farms = markers.map(m => m.farmData).filter(Boolean);
  if (!farms.length) return;

  const slidesHTML = farms.map(f =>
    `<div class="cluster-popup-slide">${buildFarmPopupHTML(f)}</div>`
  ).join('');

  const dotsHTML = farms.length > 1
    ? `<div class="cluster-dots" id="clusterDots">${farms.map((_, i) => `<span class="cluster-dot${i===0?' active':''}"></span>`).join('')}</div>`
    : '';

  const html = `<div class="cluster-carousel" id="clusterCarousel">${slidesHTML}</div>${dotsHTML}`;

  openPinCard(html, {
    hideClose: true,
    onOpen: function() {
      _initClusterCarouselDots();
      farms.forEach(loadFarmPopupStars);
    }
  });
}

function _initClusterCarouselDots() {
  const track = document.getElementById('clusterCarousel');
  if (!track) return;
  _clusterCarouselEl = track;
  track.addEventListener('scroll', () => {
    clearTimeout(track._dotsDebounce);
    track._dotsDebounce = setTimeout(_updateClusterDots, 80);
  }, { passive: true });
}

function _updateClusterDots() {
  const track = _clusterCarouselEl;
  if (!track) return;
  const dotsEl = document.getElementById('clusterDots');
  if (!dotsEl || !track.clientWidth) return;
  const idx = Math.round(track.scrollLeft / track.clientWidth);
  dotsEl.querySelectorAll('.cluster-dot').forEach((d, i) => d.classList.toggle('active', i === idx));
}
...
// Called by the popup's "View details" button (single pin popup AND each
// cluster carousel slide). Instead of an abrupt page swap, a clone of the
// pin card visually grows to fill the screen, then the real detail page
// swaps in underneath once it's fully expanded.
//
// Note: the expand target is set via direct inline-style changes (not a
// CSS class) — the starting rect is also set inline, and inline styles
// always beat a class rule for the same property, so a class toggle alone
// would never actually animate anything.
function openFarmDetailFromPopup(id) {
  const card = document.getElementById('pinCard');
  if (!card || !card.classList.contains('open')) { openFarmDetail(id); return; }

  const rect = card.getBoundingClientRect();
  const overlay = document.createElement('div');
  overlay.className = 'farm-detail-transition';
  ...
  // Hide the real popup immediately — the overlay takes over visually.
  closePinCard();

  // Force a layout flush so the browser registers the starting rect above
  // before the next style change, otherwise there's nothing to animate from.
  ...
  const finish = () => {
    ...
    openFarmDetail(id);
    // Let the detail page paint one frame before fading the overlay out,
    // so there's no flash of it underneath mid-transition.
  ...
  const onTransitionEnd = (e) => {
    if (e.target === overlay && (e.propertyName === 'width' || e.propertyName === 'height')) finish();
  };
  overlay.addEventListener('transitionend', onTransitionEnd);
  // Safety net — if transitions are disabled/skipped for any reason
  // (reduced-motion setting, a dropped frame, etc.) still open the page.
