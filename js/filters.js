...
function applyAdvFilter() {
  ...
  if (advContext === 'farms') {
    ...
    // same way the old on-map dropdown did.
    let overridePrices;
    if (singleType && s.subFilters.length) {
      const sliderVal = parseInt(document.getElementById('priceRangeInput').value);
      overridePrices = _allPricesMaster.filter(p => {
        const matchCat = normalizePriceCat(p.item_category_first) === singleType;
        const matchItem = s.subFilters.includes((p.item_name||'').trim());
