...
async function loadBestDealsCarousel() {
  ...
  try {
    ...
    let deals = [];
    try {
      const rows = await supaFetchLimit('deals', 'status=in.(approved,negotiating)&select=*,venues(*)&order=created_at.desc&limit=60');
      deals = rows.map(flattenVenue);
    ...
    let prices = [];
    try {
      const rows = await supaFetchLimit('local_prices', 'status=eq.approved&select=item_category_first,price_normal,item_name,venues(business_name,city,country)&order=price_normal.asc&limit=60');
      prices = rows.map(flattenVenue);
    } catch(e) {}
