...
function initDealMap() {
  dealsLoaded = true;
  window.dealMap = dealMap = L.map('dealMap', {center:[-30,147], zoom:4, maxZoom:15, zoomControl:false});
...
function buildDealOfferHTML(d) {
  ...
  return `
    ...
    <button class="popup-btn" onclick="openDealDetail('${d.id}')">View deal details</button>`;
}
  ...
  allKeys.forEach(key => {
    ...
    function buildPopupHTML() {
      ...
      if (priceGroup) {
        ...
        html += `<table style="width:100%;margin:8px 0;">${rowsHtml}</table>${extraInfo}<button class="popup-btn" onclick="openLocalPriceDetail(decodeURIComponent('${safeKey}'))">View prices</button>`;
      }
      html += '</div>';
      return html;
    }
